...
*/
	...
	for (var i in region.startCss) {
		...
		if (i == 'backgroundPosition') {
			$(elm).stop().animate({'background-position-x': '50%', 'background-position-y': target}, 750, 'linear');
		} else {
			$(elm).stop().animate(json, 750, 'linear');
		}
	}
}
...
function fnLoad() {
	...
	var boxHeight = (windowHeight-60)>600? (windowHeight-60) : 600;  // 數字越大，版塊越下移
...
function boxGoIn_2() {
	 box2In = false;
	$(".b2_01").stop().animate({left: 0, top: 110, opacity: 1}, 800);
